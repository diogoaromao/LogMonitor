# Paths
$projects_path = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent
#Write-Host ('Projects Path') -f 'green' $projects_path

$logmonitor_path = Split-Path $PSScriptRoot -Parent
...
$unittest_dll_path = ($logmonitor_path +'\LogMonitor.UnitTesting\bin\Release\LogMonitor.UnitTesting.dll')
